...
export default function PriceCard({ title, icon, data, type, onToggle, change }) {
  const changeMarker =
    change?.direction === 'up' ? '^' : change?.direction === 'down' ? 'v' : '*'
  ...
  return (
    <div className="glass-card p-8 w-full">
      ...
      <div className="space-y-4">
        <div className="glass p-4 rounded-xl">
            ...
            <div className="text-right">
              ...
              {change && (
                <p className={`text-xs ${getChangeColor()} mt-1`}>
                  {changeMarker} ${Math.abs(Number(change.change))} ({change.percentage}%)
                </p>
              )}
            </div>
          </div>
          <div className="flex justify-between items-center">
            ...
            <span className="text-white font-mono text-xl">Rs. {data?.usdInr || '--'}</span>
          </div>
        </div>
          ...
          {type === 'gold' ? (
            <>
              <div className="flex justify-between items-center mb-3">
                ...
                <span className="text-yellow-400 font-bold text-xl">Rs. {data?.rates?.['24K'] || '--'}</span>
              </div>
              <div className="flex justify-between items-center mb-3">
                <span className="text-gray-300">22K Gold</span>
                <span className="text-yellow-300 font-bold text-xl">Rs. {data?.rates?.['22K'] || '--'}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-300">18K Gold</span>
                <span className="text-yellow-200 font-bold text-xl">Rs. {data?.rates?.['18K'] || '--'}</span>
              </div>
            </>
          ) : (
            <>
              <div className="flex justify-between items-center mb-3">
                ...
                <div className="text-right">
                  <span className="text-gray-200 font-bold text-2xl">Rs. {data?.ratePerGram || '--'}</span>
                  {change && (
                    <p className={`text-xs ${getChangeColor()} mt-1`}>
                      {changeMarker} Rs. {Math.abs(Number(change.change))} ({change.percentage}%)
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="mt-3 pt-3 border-t border-white/10">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-white font-mono">Rs. {data?.perKg || '--'}</span>
                </div>
              </div>
            </>
          )}
        ...
        <div className="text-center text-gray-400 text-xs mt-4">
          <p>Live updates every 10 seconds from the configured providers</p>
      ...
      >
        <span aria-hidden="true">&lt;&gt;</span> Toggle {type === 'gold' ? 'Silver' : 'Gold'} Rates
      </button>
    </div>
  )
}
